...
export default function ReassignAssetsModal({
  ...
  const reassignItem = async (itemId: number, targetId: number) => {
    await axios.put(route('assignments.reassignItem', { item: itemId }), {
      new_personnel_id: targetId,
    });

    // Remember the selection in the row, and refresh the data
    setRowSelections((prev) => ({ ...prev, [itemId]: targetId }));
    fetchAssets(page, assignmentId ?? null);
  };

  const bulkReassign = async () => {
    if (!fromPersonnel || !toPersonnel) return;
    await axios.put(
      route('assignments.bulkReassign', { personnel: fromPersonnel }),
                            ...
                            <div className="w-72 mx-auto">
                              <Select
                                ...
                                onChange={(opt) => {
                                  ...
                                  if (newId) reassignItem(a.id, newId);
                                }}
