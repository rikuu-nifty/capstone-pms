...
function formatStatusLabel(label: string): string {
    return STATUS_LABELS[label.toLowerCase()] 
...
export default function InventorySheetReport() {
    const { chartData, assets: initialAssets, buildings, departments, rooms, subAreas } =
    ...
    const requireSubAreaToFetch = false; // flip to true if you want sub-area to be mandatory

    // Auto-generate when scope is ready
    useEffect(() => {
        const hasBuilding = !!filters.building_id;
        const hasRoom = !!filters.room_id;
        const hasSubArea = !!filters.sub_area_id;

        const shouldFetch = requireSubAreaToFetch
        ? hasBuilding && hasRoom && hasSubArea
        : hasBuilding && hasRoom;
                    ...
                    <Card className="rounded-2xl shadow-md">
                        ...
                        <CardContent className="h-96">
                            {viewMode === 'chart' ? (
                                        ...
                                        <div className="mt-5 flex flex-wrap justify-center gap-4 text-sm">
                                            {unsortedChartData.map((entry, index) => (
                                                <div key={index} className="flex items-center gap-2">
                                                    ...
                                                    />
                                                    <span className="text-gray-700">{formatStatusLabel(entry.label)}</span>
                                ...
                                <InventorySheetTable assets={displayedAssets} />
                            )}
                        </CardContent>
                    </Card>
            </div>
