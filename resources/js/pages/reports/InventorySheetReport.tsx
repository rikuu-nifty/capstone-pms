...
// 🔹 Placeholder for your tables (you’ll implement later)
const PlaceholderTable = ({ label }: { label: string }) => (
    <div className="flex h-full items-center justify-center text-gray-400">
        <p className="text-sm">{label} – table view coming soon.</p>
    </div>
)
...
type InventorySheetPageProps = {
    chartData: ChartData[]
    sheets: any[] // 🔹 replace with your real sheet records later
...
export default function InventorySheetReport() {
    const { chartData, sheets, buildings, departments } = usePage<InventorySheetPageProps>().props

    const [viewMode, setViewMode] = useState<'chart' | 'table'>('chart')

    // 🔹 Filters (basic for now — extend later)
    ...
    return (
        ...
        <div className="space-y-6 px-6 py-4">
            ...
            <p className="text-sm text-muted-foreground">
                Generate detailed inventory sheets grouped by unit, building, and room.
            </p>
            </div>
            ...
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 md:grid-cols-3">
                ...
                <Select
                    ...
                    value={
                    filters.building_id
                        ? { value: filters.building_id, label: buildings.find((b) => b.id === filters.building_id)?.name || '' }
                        : null
                    }
            ...
            <CardHeader className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="inline-flex rounded-md shadow-sm">
                    ...
                    className={`border px-4 py-2 text-sm font-medium ${
                        viewMode === 'chart' ? 'border-blue-600 bg-blue-600 text-white' : 'border-gray-300 bg-white text-gray-700 hover:bg-gray-50'
                    } rounded-l-md`}
                    >
                    Chart
                    </button>
                    ...
                    className={`border-t border-b px-4 py-2 text-sm font-medium ${
                        viewMode === 'table' ? 'border-blue-600 bg-blue-600 text-white' : 'border-gray-300 bg-white text-gray-700 hover:bg-gray-50'
                    } rounded-r-md`}
                    >
                    Table
                    </button>
            ...
            <CardContent className="h-96">
                ...
                totalAssets === 0 ? (
                    ...
                    >
                        ...
                        <Pie data={unsortedChartData} dataKey="value" nameKey="label" innerRadius={80} outerRadius={120} stroke="none">
                    ...
                    <div className="mt-5 flex flex-wrap justify-center gap-4 text-sm">
                        ...
                        <div key={index} className="flex items-center gap-2">
                            <span className="h-3 w-3 rounded-sm" style={{ backgroundColor: generateColor(index, unsortedChartData.length) }} />
                ...
                <PlaceholderTable label="Inventory Sheets" />
                )}
            </CardContent>
            </Card>
        </div>
