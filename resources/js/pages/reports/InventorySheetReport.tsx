...
import { Area, AreaChart, CartesianGrid, LabelList, LabelProps, XAxis, YAxis } from 'recharts';
...
function formatStatusLabel(label: string): string {
    return STATUS_LABELS[label.toLowerCase()] ?? label.replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase());
}
...
export default function InventorySheetReport() {
    const { chartData, assets, buildings, departments, rooms, subAreas } = usePage<InventorySheetPageProps>().props;
    ...
    const filteredRooms = filters.building_id ? rooms.filter((r) => r.building_id === filters.building_id) : rooms;

    const filteredSubAreas = filters.room_id ? subAreas.filter((s) => s.building_room_id === filters.room_id) : subAreas;
    ...
    const filteredData: ChartData[] = React.useMemo(() => {
        ...
        for (let d = new Date(start); d <= referenceDate; d.setDate(d.getDate() + 1)) {
            ...
            filled.push(dataMap.get(key) || { date: key, inventoried: 0, scheduled: 0, not_inventoried: 0 });
        }
    ...
    const chartHasAnyValues = filteredData.some((d) => (d.inventoried || 0) + (d.scheduled || 0) + (d.not_inventoried || 0) > 0);
    ...
    function renderTotalLabel(props: LabelProps & { index?: number; data?: ChartData[] }) {
        ...
        const d = data[index];
        const total = (d.inventoried || 0) + (d.scheduled || 0) + (d.not_inventoried || 0);

        if (total === 0) return null;

        return (
            <text x={Number(x)} y={Number(y) - 6} textAnchor="middle" fontSize={12} fill="#374151" fontWeight="500">
                {total}
            </text>
        );
    }
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="space-y-6 px-6 py-4">
                ...
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">Generate detailed per-room/per-building inventory sheets.</p>
                </div>
                ...
                <div className="space-y-6 rounded-xl border bg-white p-6 shadow-sm">
                    ...
                    <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                        ...
                        {/* Inventory Status */}
                        <div>
                            <label className="mb-1 block text-sm font-medium text-gray-700">Inventory Status</label>
                            <Select
                                className="w-full"
                                isClearable
                                value={
                        ...
                        {/* Sub-Area */}
                        <div>
                            <label className="mb-1 block text-sm font-medium text-gray-700">Sub-Area</label>
                            <Select
                                className="w-full"
                                isClearable
                                value={
                            ...
                            <PopoverContent align="end" className="w-44 p-2">
                                ...
                                <button
                                    onClick={() => {
                                        ...
                                        window.open(route('reports.inventory-sheet.export.pdf') + '?' + query, '_blank');
                                ...
                                <button
                                    onClick={() => {
                                        const query = buildQuery(filters);
                                        window.open(route('reports.inventory-sheet.export.excel') + '?' + query, '_blank');
                    ...
                    <CardHeader className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <div className="mt-2 flex items-center gap-2 sm:mt-0">
                            {viewMode === 'chart' && (
                                <ShadSelect
                                    value={timeRange}
                                    onValueChange={(v) => setTimeRange((v as '7d' | '30d' | '90d' | '18w' | '1y') ?? '90d')}
                                >
                    ...
                    <CardContent className="h-96">
                        {viewMode === 'chart' ? (
                            filteredData.length === 0 || !chartHasAnyValues ? (
                                ...
                                >
                                    <AreaChart data={filteredData} margin={{ right: 25, left: 20, top: 15 }}>
                                        <defs>
                    ...
                    />
                        ...
                        <ChartLegend content={<ChartLegendContent />} /> */}
                                        ...
                                        <Area
                                            ...
                                            stackId="a"
                                        />
                                        <Area dataKey="inventoried" type="monotone" fill="url(#fillInventoried)" stroke="#00A86B" stackId="a" />
                                        {/* <Area
                          dataKey="scheduled"
                          type="monotone"
                          fill="url(#fillScheduled)"
                          ...
                          stackId="a"
                        /> */}
                                        <Area dataKey="scheduled" type="monotone" fill="url(#fillScheduled)" stroke="#3b82f6" stackId="a">
                                            <LabelList dataKey="scheduled" content={(props) => renderTotalLabel({ ...props, data: filteredData })} />
                                        </Area>
